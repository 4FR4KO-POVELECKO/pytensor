...
class TestOptimizer:
    ...
    def test_rmsprop(self):
        ...
        test_x = self.x.data - (self.x.grad.data * optimizer.lr) / (np.sqrt(m_avg) + optimizer.epsilon)
        optimizer.step()
        assert all(self.x.data == test_x)
    ...
    def test_adam(self):
        ...
        v = optimizer.beta_2 * 0 + (1.0 - optimizer.beta_2) * np.sqrt(self.x.grad.data)
        m_hat = m / (1.0 - optimizer.beta_1 ** 0+1)
        v_hat = v / (1.0 - optimizer.beta_2 ** 0+1)
        test_x = self.x.data - (optimizer.lr * m_hat / (np.sqrt(v_hat) + optimizer.epsilon))
        optimizer.step()
        assert all(self.x.data == test_x)
